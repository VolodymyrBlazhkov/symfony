...
use App\Exception\BookLevelMaxException;
...
use App\Modal\BookChapter as BookChapterModel;
...
class AuthorBookChaperService
{
    ...
    public function createChapter(CreateBookChapterRequest $request, int $id): IdResponse
    {
        ...
        if ($parentId !== null) {
            ...
            if ($parentLevel === self::MAX_LEVEL) {
                throw new BookLevelMaxException();
            }
    ...
    public function updateChapterSort(UpdateBookChapterSortRequest $request): void
    {
        ...
        if (SorPosition::AsLast === $sortContext->getPosition()) {
    ...
    public function getNextMaxSort(Book $book, int $level): int
    {
