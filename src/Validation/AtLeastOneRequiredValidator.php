...
class AtLeastOneRequiredValidator extends ConstraintValidator
{
    ...
    public function validate(mixed $ob, Constraint $constraint)
    {
        if (!$constraint instanceof AtLeastOneRequired) {
            throw new UnexpectedTypeException($constraint, AtLeastOneRequired::class);
        }

        $passed = array_filter($constraint->requiredFields, function (string $required) use ($ob) {
            return $this->propertyAccessor->getValue($ob, $required) !== null;
        });
        ...
        foreach ($constraint->requiredFields as $required) {
            $this->context->buildViolation($constraint->message)
                ...
                ->setCode(AtLeastOneRequired::ONE_REQ_ERROR)
                ->atPath($required)
                ->addViolation();
        }
    }
